...
/// Incremental SHA-256, so a session that reaches tens of megabytes never has
/// to sit in memory whole just to be fingerprinted or archived.
///
/// Same compression function as the one-shot [`sha256`] (which is now
/// implemented in terms of this), so the FIPS 180-4 vectors checked against it
/// cover this path too -- confirmed directly by
/// `streaming_the_same_bytes_in_small_chunks_yields_the_same_digest_as_one_shot`
/// below.
pub(crate) struct Sha256 {
    state: [u32; 8],
    /// Bytes not yet folded into `state`: fewer than one 64-byte block.
    buffer: [u8; 64],
    buffer_len: usize,
    /// Total bytes seen, for the length suffix FIPS 180-4 padding appends.
    total_len: u64,
}
...
/// Lowercase hex encoding of a digest, as archive manifests store it.
...
mod tests {
    ...
    #[test]
    fn streaming_the_same_bytes_in_small_chunks_yields_the_same_digest_as_one_shot() {
        // The whole point of the incremental hasher: ingest must be able to
        // feed it a session a few kilobytes at a time and still land on the
        // digest a one-shot hash of the same bytes would produce. The input is
        // deliberately longer than one 64-byte block and not a multiple of the
        // chunk size, so it exercises the buffered remainder path too.
